...
|---|---|---|
| `henax-*` | **orizzontale / riusabile** — portabile su qualunque cliente, dipende solo da librerie piattaforma + Dolibarr core | `henax-ai`, `henax-chat`, `henax-fse`, `henax-export`, `henax-analytics`, `henax-help`, `henax-admin`, `henax-geo`, `henax-ops` |
| `domicare-*` | **verticale Domicare** — specifico del cliente Consorzio Domicare | `domicare-core`, `domicare-rendicontazione`, `domicare-compliance`, `domicare-docai`, `domicare-woundcare`, `domicare-hcp` |
...
4. **Prefisso tabelle** = nome modulo con `_`: `henax-chat` ⇒ `llx_henaxchat_*`. Una tabella vive in UN solo modulo.
